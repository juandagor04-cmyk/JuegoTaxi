...
public class SenalTransito {
    ...
    public static final int VELOCIDAD = 5; // <--- Agregado de la segunda versión
    ...
    private int tipo;
    private int subTipo;
    private int valor; // Para el límite de velocidad (e.g., 60, 40)
    ...
    // Constructor secundario para señales sin valor (como PARE o Semáforo)
    ...
    public void dibujar(Graphics g) {
        ...
        if (tipo == REGLAMENTARIA) {
            if (subTipo == SEMAFORO) {
                ...
                dibujarVelocidadPro(g2d); // <--- Nuevo diseño integrado
            }
    ...
    private void dibujarSemaforoPro(Graphics2D g2d) {
        ...
        if(estadoSemaforo == ROJO) {
            g2d.setPaint(new RadialGradientPaint(new Point2D.Float(posX+6, y+7), 8, new float[]{0f, 1f}, new Color[]{Color.WHITE, Color.RED}));
        }
        ...
        if(estadoSemaforo == AMARILLO) {
            g2d.setPaint(new RadialGradientPaint(new Point2D.Float(posX+6, y+22), 8, new float[]{0f, 1f}, new Color[]{Color.WHITE, Color.YELLOW}));
        }
        ...
        if(estadoSemaforo == VERDE) {
            g2d.setPaint(new RadialGradientPaint(new Point2D.Float(posX+6, y+37), 8, new float[]{0f, 1f}, new Color[]{Color.WHITE, Color.GREEN}));
        }
    ...
    private void dibujarParePro(Graphics2D g2d) {
        ...
        pare.addPoint(x + 10, y + 7); pare.addPoint(x + 20, y + 7);
        pare.addPoint(x + 27, y + 14); pare.addPoint(x + 27, y + 23);
        pare.addPoint(x + 20, y + 30); pare.addPoint(x + 10, y + 30);
        pare.addPoint(x + 3, y + 23); pare.addPoint(x + 3, y + 14);

        g2d.setPaint(new GradientPaint(x+3, y+7, new Color(255, 50, 50), x+27, y+30, new Color(150, 0, 0)));
        g2d.fillPolygon(pare);
        g2d.setColor(Color.WHITE);
        g2d.setStroke(new BasicStroke(2f));
        g2d.drawPolygon(pare);

        g2d.setFont(new Font("Arial", Font.BOLD, 8));
        g2d.drawString("PARE", x + 6, y + 22);
    }

    private void dibujarVelocidadPro(Graphics2D g2d) {
        // Poste
        g2d.setColor(Color.GRAY);
        g2d.fillRect(x + 13, y + 25, 3, 40);

        // Círculo Blanco con borde Rojo
        g2d.setColor(Color.WHITE);
        g2d.fillOval(x + 2, y + 2, 26, 26);
        g2d.setColor(Color.RED);
        g2d.setStroke(new BasicStroke(3f));
        g2d.drawOval(x + 2, y + 2, 26, 26);
    ...
    private void dibujarPreventivaPro(Graphics2D g2d) {
        ...
        g2d.fillPolygon(diamante);
        g2d.setColor(Color.BLACK);
        g2d.setStroke(new BasicStroke(2f));
        g2d.drawPolygon(diamante);

        if (subTipo == CURVA) {
            g2d.drawArc(x + 8, y + 10, 15, 15, 0, 150);
        } else if (subTipo == CRUCE) {
            g2d.drawLine(x + 15, y + 8, x + 15, y + 22);
            g2d.drawLine(x + 8, y + 15, x + 22, y + 15);
        }
    }

    private void dibujarInformativaPro(Graphics2D g2d) {
        g2d.setColor(new Color(0, 70, 180));
        g2d.fillRoundRect(x, y, 28, 28, 5, 5);
        g2d.setColor(Color.WHITE);
        g2d.setStroke(new BasicStroke(1.5f));
        g2d.drawRoundRect(x + 2, y + 2, 24, 24, 3, 3);

        if(subTipo == CLIENTE) {
            g2d.fillOval(x + 11, y + 7, 6, 6); // Cabeza
            g2d.fillRect(x + 9, y + 14, 10, 8); // Cuerpo
        }
    }
    ...
    public void actualizar() {
        ...
        if (subTipo == SEMAFORO) {
            if (estadoSemaforo == ROJO && tiempoCambio >= duracionRojo) {
                ...
                estadoSemaforo = ROJO; tiempoCambio = 0;
            }
        }
    }

    public void aplicarReglas(Taxi taxi) {
        if (!taxi.getBounds().intersects(new Rectangle(x, y, 30, 30))) return;

        if (subTipo == SEMAFORO && estadoSemaforo == ROJO) {
            taxi.setMulta(true);
            taxi.setPuntos(-10);
        } else if (subTipo == PARE) {
            taxi.setMulta(true);
            ...
            if (taxi.getVelocidad() > valor) {
                taxi.setMulta(true);
                System.out.println("Exceso de velocidad! Límite: " + valor);
            }
